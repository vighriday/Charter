...
export const SETTINGS: readonly Setting[] = [
  ...
  },
  {
    ...
    what: [
      ...
      '',
      'Which is why the default is the LARGEST number any stage asks for, and not a',
      'smaller round one. It was 12 for a while, and the stage that writes the',
      'contract asks for 20, so that stage was quietly given 12 and stopped part way',
      'through on the deployed site with nothing to show for it. A default tighter',
      'than the strictest stage is not a safety net, it is a silent override of a',
      'decision somebody made on purpose. A test keeps the two in step.',
    ],
    kind: 'number',
...
export function readSettings(env: Record<string, string | undefined> = process.env): Settings {
  ...
  return {
    ...
    maxTurnsPerStage: number('MAX_TURNS_PER_STAGE', 20),
